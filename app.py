...
def start_multimon(cfg):
    ...
    call = "rtl_fm -M fm -E dc -F 0 -d {} -l {} -g {} -p {} -f {} -s {} | multimon-ng -t raw {} -f alpha /dev/stdin -".format(cfg.get('rtl_fm', 'device_index'), cfg.get('rtl_fm', 'squelch_level'), cfg.get('rtl_fm', 'gain'), cfg.get('rtl_fm', 'ppm_error'), cfg.get('Frequencies', 'freq'), cfg.get('rtl_fm', 'sample_rate'), prots)
